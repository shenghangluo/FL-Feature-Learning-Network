    ...
    def train_model(self):
        ...
        with tf.Session() as sess:
            ...
            if self._Train_Test == 0:
              ...
              for pruning_iteration in range(pruning_iterations + 1):
                  ...
                  for epoch in range(0, epoch_prune):
                      ...
                      for X_batch, Rx_batch, Rx_Y_batch, Tx_Y_batch, Tx_batch in self._dataset.shuffle_batch(data['X'], data['Rx'], data['Rx_Y'], data['Tx_Y'], data['Tx'], self._batch_size):
                          ...
                          _, loss_batch = sess.run([training_op, loss], feed_dict={self._model.rate: 0.5, self._pruning_factor: pruning_factor,
                                                                                   self._learning_rate: learning_rate,
                                                                                   self._model.X: X_batch,
                                                                                   self._model.Rx: Rx_batch,
                                                                                   self._model.Rx_Y: Rx_Y_batch,
                                                                                   self._model.Tx_Y: Tx_Y_batch,
                                                                                   self._model.Tx: Tx_batch})
                          train_loss += loss_batch
                          iterations += 1
                          ii += 1
                      valid_loss = sess.run(loss,
                                            feed_dict={self._model.rate: 0.0, self._pruning_factor: pruning_factor,
                                                       self._learning_rate: learning_rate,
              ...
              for idx, weights in enumerate(weight_matrices):
                  np.savetxt('./weight_'+ str(idx) +'.csv', weight_matrices[idx], delimiter=',')

              learning_rate = 0.0
              test_point = self._model.test_point.eval(
                  feed_dict={self._model.rate: 0.0, self._learning_rate: learning_rate,
                             self._pruning_factor: pruning_factors[0], self._model.X: data['X_test'],
                             self._model.Rx: data['Rx_test'], self._model.Rx_Y: data['Rx_Y_test'],
                             self._model.Tx_Y: data['Tx_Y_test'], self._model.Tx: data['Tx_test'],
                             self._model.state: data['state']})
              real = test_point[:, 0]
              image = test_point[:, 1]
              np.savetxt('./test_real.csv', real, delimiter=',')
              ...
              NLCpred_train = self._model.get_output().eval(
                  feed_dict={self._model.rate: 0.0, self._learning_rate: learning_rate, self._pruning_factor:pruning_factors[0], self._model.X: data['X_test'],
                             self._model.Rx: data['Rx_test'], self._model.Rx_Y: data['Rx_Y_test'],
                             self._model.Tx_Y: data['Tx_Y_test'], self._model.Tx: data['Tx_test'], self._model.state: data['state']})
