...
Tx_Pow_dBm = [2] %Transmission power per channel
BaudRate = 32e9;
...
% Random interleaver
Intlv_State = randi([0, 2^15], No_sub_channels, 2);  % Interleaver can take any state betweeen 0, 2^32

%% LDPC Code Encoding Decding Initialization
Pr = log2(M)*N_data*6; % The number six represents the total number of blocks of data each with a size 2^15 
...
for pow_loop = 1:1:length(Tx_Pow_dBm)
...
%             Tx_BitsX(1, :) = randi([0,1], 1, Pr*No_of_code_blocks); % Random bit generation
%             Tx_BitsY(1, :) = randi([0,1], 1, Pr*No_of_code_blocks); % Random bit generation
%             
% 
...
%         end
%         save ('D:\MASC\Perturbation_based\learned_AM_Model\MATLAB_CODE\Tx_BitsX.txt', 'Tx_BitsX','-ascii');
%         save ('D:\MASC\Perturbation_based\learned_AM_Model\MATLAB_CODE\Tx_BitsY.txt', 'Tx_BitsY','-ascii');
...
% 
% % IMPORTANT: The testing data are saved in a folder with path
% % "G:\MASc\Perturbation_based\learned_A_M_Model\Dataset".
...
%% Testing Data
Tx_BitsX=Tx_BitsX_actual((log2(M)*4*N_data+1:end-(log2(M)*N_data)));
Tx_BitsY=Tx_BitsY_actual((log2(M)*4*N_data+1:end-(log2(M)*N_data)));
Rx_SymbolsX_Com=Rx_SymbolsX_Com_actual((4*N_data+1:end-(N_data)));
Rx_SymbolsY_Com=Rx_SymbolsY_Com_actual((4*N_data+1:end-(N_data)));
Input_Symbol_real_X=Input_Symbol_real_X_actual((4*N_data+1:end-(N_data)));
Input_Symbol_image_X=Input_Symbol_image_X_actual((4*N_data+1:end-(N_data)));
Transmit_real_X=Transmit_real_X_actual((4*N_data+1:end-(N_data)));
Transmit_image_X=Transmit_image_X_actual((4*N_data+1:end-(N_data)));

save('G:\MASc\Perturbation_based\learned_A_M_Model\Dataset\Dataset_16QAM_3200_Dual_test_2_1.mat','Tx_BitsX','Tx_BitsY','Rx_SymbolsX_Com','Rx_SymbolsY_Com', 'Tx_Pow_dBm', 'Input_Symbol_real_X','Input_Symbol_image_X','Transmit_real_X','Transmit_image_X');         
end
