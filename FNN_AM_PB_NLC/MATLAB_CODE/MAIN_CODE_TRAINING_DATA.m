...
%% ---------------------------------------------------------------------------------------------------------------------------------- %% 
        ...
        Tx_SymbolsX(1, :) = transpose(qammod(Tx_BitsX(1, 1:log2(M)*N_data).', M,  'InputType', 'bit', 'UnitAveragePower', true));  %M QAM modulation 
        Tx_SymbolsY(1, :) = transpose(qammod(Tx_BitsY(1, 1:log2(M)*N_data).', M,  'InputType', 'bit', 'UnitAveragePower', true));   %M QAM modulation       
...
%         load ('Pilot_SymbolsX_bit.txt', 'Pilot_SymbolsX_bit','-ascii');
%         load ('Pilot_SymbolsY_bit.txt', 'Pilot_SymbolsY_bit','-ascii');
% 
...
%         save ('C:\Sunish\Post_Doc_Work_UBC_2020\DNN_SO_PB_NLC_CODEBASE\MATLAB_CODE\Pilot_SymbolsX_bit.txt', 'Pilot_SymbolsX_bit','-ascii');
%         save ('C:\Sunish\Post_Doc_Work_UBC_2020\DNN_SO_PB_NLC_CODEBASE\MATLAB_CODE\Pilot_SymbolsY_bit.txt', 'Pilot_SymbolsY_bit','-ascii');

%% ---------------------------------------------------------------------------------------------------------------------------------- %% 

%         load ('C:\Sunish\Post_Doc_Work_UBC_2020\DNN_SO_PB_NLC_CODEBASE\MATLAB_CODE/Pilot_SymbolsX_bit.txt', 'Pilot_SymbolsX_bit','-ascii');
%         load ('C:\Sunish\Post_Doc_Work_UBC_2020\DNN_SO_PB_NLC_CODEBASE\MATLAB_CODE/Pilot_SymbolsY_bit.txt', 'Pilot_SymbolsY_bit','-ascii');
% 
%         Pilot_SymbolsX = qammod(Pilot_SymbolsX_bit, M,  'InputType', 'integer', 'UnitAveragePower', true);
%         Pilot_SymbolsY = qammod(Pilot_SymbolsY_bit, M,  'InputType', 'integer', 'UnitAveragePower', true);
        ...
        COI_no =(No_sub_channels+1)/2;  % Need to define here for ISI estimation
...
for kk=1:1:5 % This is to transmit ONLY the training data five times and take the average at the receiver for the denoising
...
%% ----------Please activate this section if you want to have 50% pre-CDC (also set 1*Chrom_Disp=0.5*Chrom_Disp in the input argument of the 
% transmitter and receiver "CD_Compensation_Lumped" function--------- %%
        %% CD compensation 
...
%         my_plot_fft(Rx_Symbols_Channel(1,:), Total_upsamling_factor, BaudRate, 'norm')
        ...
        clear  Tx_SymbolsY_Dig_upsamples_norm_temp;
         ...
         Rx_SymbolsX_Filt_norm=mean(Rx_SymbolsX_Filt_norm);
         Rx_SymbolsY_Filt_norm=mean(Rx_SymbolsY_Filt_norm);
...
%% Training data
Tx_BitsX=Tx_BitsX_actual(1:log2(M)*N_data);
Tx_BitsY=Tx_BitsY_actual(1:log2(M)*N_data);
Rx_SymbolsX_Com=Rx_SymbolsX_Com_actual(1:N_data);
Rx_SymbolsY_Com=Rx_SymbolsY_Com_actual(1:N_data);
Input_Symbol_real_X=Input_Symbol_real_X_actual(1:N_data);
Input_Symbol_image_X=Input_Symbol_image_X_actual(1:N_data);
Transmit_real_X=Transmit_real_X_actual(1:N_data);
Transmit_image_X=Transmit_image_X_actual(1:N_data);
save('D:\MASC\Perturbation_based\learned_AM_Model\Dataset\Dataset_Dual_train_1_PB.mat','Tx_BitsX','Tx_BitsY','Rx_SymbolsX_Com','Rx_SymbolsY_Com', 'Tx_Pow_dBm', 'Input_Symbol_real_X','Input_Symbol_image_X','Transmit_real_X','Transmit_image_X');         
